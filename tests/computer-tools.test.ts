...
test("visible directory tools read only from explicitly granted local directories", async () => {
  ...
  try {
    ...
    const found = await find.tool.execute(
      ...
      find.input,
    ) as { schema: string; rootId: string; matches: string[]; evidenceReceipt: { schema: string; sourceRefs: Array<{ path: string }> } };
    assert.equal(found.schema, "agentloop.visibleFindFiles/v1");
    assert.deepEqual(found.matches, ["materials/brief.md"]);
    assert.deepEqual({
      rootId: found.rootId,
      limit: (found as { limit: number }).limit,
      returned: (found as { returned: number }).returned,
      totalMatches: (found as { totalMatches: number }).totalMatches,
      truncated: (found as { truncated: boolean }).truncated,
    }, {
