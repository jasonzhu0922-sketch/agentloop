...
test("read_source returns uploaded chunk content with receipt coverage and explicit caveats", async () => {
  ...
  try {
    ...
    const owner = await auth.register("source-tool@example.com", "source tool secure password");
    const repository = new SourceRepository(database);
    const now = Date.now();
    const source = repository.insertSource({
    ...
    repository.replaceChunks(source.id, [{
      chunk_index: 0,
      kind: "text",
      locator: "chars=1-22",
      content: "task,owner\nrisk,alice\n",
...
test("read_source treats chunkIndex and maxChunks as a consecutive read window", async () => {
  ...
  try {
    const auth = new AuthService(database);
    const owner = await auth.register("source-window@example.com", "source window secure password");
    const repository = new SourceRepository(database);
    const now = Date.now();
    const source = repository.insertSource({
