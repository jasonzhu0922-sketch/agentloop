...
test("an expired dispatched Action becomes recovery_required without terminalizing its Run", async () => {
  ...
  try {
    ...
    `).run(runId, owner.user.id, agent.id, "test action recovery", Date.now());
    ...
    database.raw.prepare("UPDATE runtime_actions SET deadline_at = 0, lease_until = 0 WHERE id = ?")
      .run(action.id);
    ...
    const run = database.raw.prepare("SELECT status FROM runs WHERE id = ?").get(runId) as { status: string };
    assert.equal(run.status, "running");
    const outcome = database.raw.prepare("SELECT COUNT(*) AS count FROM run_outcomes WHERE run_id = ?")
      .get(runId) as { count: number };
    assert.equal(outcome.count, 0);
    const event = database.raw.prepare("SELECT type FROM run_events WHERE run_id = ? ORDER BY seq DESC LIMIT 1")
