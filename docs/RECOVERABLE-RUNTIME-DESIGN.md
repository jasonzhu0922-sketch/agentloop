...
用户取消是另一条权威关闭路径：取消事务先把 dispatched Action 关闭为 `failed/CANCELLED` 并提交 Run 的 cancelled Outcome，再 abort 在线执行。若 Model 或 Tool 忽略 abort 并迟到，执行调用向宿主规范化返回 `CANCELLED`；只有 Run 仍为 running 且 Action fence 被恢复 owner 替换时，才保留 `RUNTIME_ACTION_LEASE_LOST`。这一区分不会允许迟到 Worker 提交 outcome，也不会削弱 reconciler fencing。
