...
import type { PrivateSkill } from "@zhujun/agentloop";
import type { RenderEnvironmentEvidence } from "@zhujun/agentloop";
...
const SKILLS_ROOT = resolve(ROOT, "packages", "agentloop-skills", "skills");
const UPSTREAM_SKILL_ROOT = resolve(SKILLS_ROOT, "presentation-skill");
const UPSTREAM_SOURCE_LOCK = resolve(SKILLS_ROOT, "presentation-skill.source.json");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const plan = await runs.plan(user.user.id, run.id);
    const events = await runs.events(user.user.id, run.id);
...
export function evaluateAcceptance(input: {
  events: Awaited<ReturnType<RunService["events"]>>;
...
export async function writeFailureEvidence(input: {
  ...
  const detectedRunId = input.runId ?? extractRunId(input.error) ?? (await findLatestRunId(input));
  const run = await collectPersisted(async () => {
  ...
  const plan = await collectPersisted(async () => {
    if (detectedRunId === undefined || input.runs === undefined || input.actorUserId === undefined) return undefined;
    return input.runs.plan(input.actorUserId, detectedRunId);
  });
  const events = await collectPersisted(async () => {
  ...
  const outcome = await collectPersisted(async () => {
...
async function findLatestRunId(input: {
  database?: AppDatabase;
  actorUserId?: string;
...
async function collectPersisted<T>(load: () => T | Promise<T>): Promise<T | undefined> {
  try {
    return await load();
  } catch {
    return undefined;
