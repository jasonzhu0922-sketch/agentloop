...
test("registration stores only password and session digests", async () => {
  ...
  try {
    ...
    assert.equal((await auth.authenticate(result.token)).id, result.user.id);

    const userRow = (await database.prepare("SELECT password_hash FROM users WHERE id = ?").get(result.user.id)) as {
      password_hash: string;
    };
    const sessionRow = (await database.prepare("SELECT token_hash FROM auth_sessions WHERE user_id = ?").get(result.user.id)) as {
      token_hash: string;
    };
    ...
    await auth.revoke(second.token);
    await assert.rejects(
...
test("private skill ownership is enforced at query boundaries", async () => {
  ...
  try {
    ...
    assert.deepEqual(await skills.list(stranger.user.id), []);
    await assert.rejects(
      () => skills.get(stranger.user.id, skill.id),
      (error: unknown) => hasErrorCode(error, "NOT_FOUND"),
    );
    assert.deepEqual((await skills.list(owner.user.id)).map((entry) => entry.id), [skill.id]);
  } finally {
    database.close();
  }
});
