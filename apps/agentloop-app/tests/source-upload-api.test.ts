...
test("source intake extracts HTML, PDF, DOCX, XLSX, and PPTX uploads into readable chunks", async () => {
  ...
  try {
    ...
    for (const item of cases) {
      ...
      assert.match(summary.summary ?? "", item.expected ?? /intake evidence|Region|North|120/, item.name);
      if (item.rejected) assert.doesNotMatch(summary.summary ?? "", item.rejected, item.name);
      const chunkText = (await sourceRepository.chunks(source.id)).map((chunk) => chunk.content).join("\n");
      assert.match(chunkText, item.expected ?? /intake evidence|Region|North|120/, item.name);
      if (item.rejected) assert.doesNotMatch(chunkText, item.rejected, item.name);
    }
  } finally {
    database.close();
    await fs.rm(workspace, { recursive: true, force: true });
