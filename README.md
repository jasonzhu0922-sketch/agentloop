...
- 启动 reconciliation 只接管 lease/deadline 已失效并通过 fence/revision CAS 取得所有权的 Tool Action；有效租约保持归原 Worker。reconciled outcome 与 `recovery_review` 在同一 SQL 事务提交；失去 fence 的旧 Worker 不得再写 Step、Plan、Run、Outcome 或矛盾的 `tool.failed`。用户取消已关闭 Run/Action 时，迟到的 Model/Tool completion 统一返回 `CANCELLED`；仍在运行且被新 owner 接管的 Action 才报告 `RUNTIME_ACTION_LEASE_LOST`。`TOOL_RESULT_SERIALIZATION_FAILED` 与 durable outcome 后的持续投影故障都先走恢复控制分支，不进入通用 `run.failed`。内核及 Multi Runtime PostgreSQL schema 的 canonical DDL、catalog 检查、必要的 BIGINT 升级和版本 ledger 在同一个 advisory-lock transaction 中串行执行；只 ALTER 仍非 BIGINT 的列，完成版本不会在后续启动重复锁业务表，空 schema 的并发首次启动也不会竞争 PostgreSQL catalog。OID 20 的 number/string/bigint 返回值都必须通过安全整数检查。
