...
export class SourceIntakeService {
  ...
  async upload(input: {
    ...
    const row = this.repository.insertSource({
      id,
    ...
    if (extraction.status === "ready") {
      this.repository.replaceChunks(id, buildChunks(extraction.text), now);
    }
    return this.summary(row);
  }

  summary(row: SourceRow): UploadedSourceSummary {
    return {
      ...sourceSummary(row),
      chunkCount: this.repository.chunks(row.id).length,
    };
  }
}
