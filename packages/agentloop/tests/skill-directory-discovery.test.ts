...
test("RunService refreshes the discovered Skill directory before planning a new Run", async () => {
  ...
  try {
    ...
    const plan = (await runs.plan(owner.user.id, run.id)).plan;
    assert.deepEqual(plan.selectedSkillIds, ["discovered:directory-demo"]);
    const selectedEvent = (await runs.events(owner.user.id, run.id)).find((event) =>
...
test("invalid Skill package errors include the package directory", async () => {
  const root = await fs.mkdtemp(resolve(tmpdir(), "agentloop-invalid-skill-directory-"));
  try {
    const skillDirectory = resolve(root, "skills");
    const badPackage = resolve(skillDirectory, "broken-demo");
    await fs.mkdir(badPackage, { recursive: true });
    await fs.writeFile(resolve(badPackage, "SKILL.md"), [
      "---",
      "AIGC:",
      "  Label: \"1\"",
      "---",
    ...
    const { discoverSkillDirectory } = await import("../src/skills/skill-directory.ts");
    const canonicalBadPackage = await fs.realpath(badPackage);
    await assert.rejects(
      () => discoverSkillDirectory(skillDirectory),
      (error: unknown) =>
        error instanceof Error
        && error.message.includes(`Invalid Skill package at ${canonicalBadPackage}`)
        && error.message.includes("frontmatter name"),
    );
  } finally {
...
test("Skill packages from multiple directories merge into one global catalog", async () => {
  const fixture = await createDirectoryFixture();
  const teamRoot = await fs.mkdtemp(resolve(tmpdir(), "agentloop-skill-directory-team-"));
  const database = new AppDatabase(":memory:");
  try {
    const teamDirectory = resolve(teamRoot, "team-skills");
    const teamPackage = resolve(teamDirectory, "team-demo");
    await fs.mkdir(teamPackage, { recursive: true });
    await fs.writeFile(resolve(teamPackage, "SKILL.md"), [
      "---",
      "name: team-demo",
      "description: A team-provided workflow from its own directory",
      "---",
      "",
      "TEAM-SKILL-BODY",
      "",
    ].join("\n"));
    const teamInspection = await inspectSkillPackage(teamPackage);


    const skills = new SkillService(database, {
      packageStoreRoot: fixture.packageStore,
      skillDirectories: [fixture.skillDirectory, teamDirectory],
    });
    assert.equal(skills.skillDirectories.length, 2);
    assert.deepEqual(await skills.refreshSkillDirectory().then((catalog) => catalog.map((item) => item.name)), [
      "directory-demo",
      "team-demo",
    ]);
    ...
    const owner = testOwner();
    const stranger = testOwner();
    for (const user of [owner.user.id, stranger.user.id]) {
      const available = await skills.listAvailable(user);
      assert.deepEqual(available.map((skill) => skill.name), ["directory-demo", "team-demo"]);
      const byName = new Map(available.map((skill) => [skill.name, skill]));
      assert.equal(byName.get("directory-demo")?.package?.root, fixture.sourcePackage);
      assert.equal(byName.get("team-demo")?.package?.root, teamInspection.root);
      assert.match(byName.get("team-demo")!.id, /^discovered:team-demo$/);
    }
  } finally {
    await database.close();
    await removeSkillPackage(fixture.packageStore).catch(() => undefined);
...
test("duplicate Skill names across directories fail discovery closed", async () => {
  const fixture = await createDirectoryFixture();
  const conflictRoot = await fs.mkdtemp(resolve(tmpdir(), "agentloop-skill-directory-dup-"));
  const database = new AppDatabase(":memory:");
  try {
    const conflictDirectory = resolve(conflictRoot, "other-skills");
    const conflictPackage = resolve(conflictDirectory, "directory-demo");
    await fs.mkdir(conflictPackage, { recursive: true });
    await fs.writeFile(resolve(conflictPackage, "SKILL.md"), [
      "---",
      "name: directory-demo",
      "description: Same name as the built-in demo package",
      "---",
      "",
      "CONFLICTING-BODY",
      "",
    ...
    const skills = new SkillService(database, {
      packageStoreRoot: fixture.packageStore,
      skillDirectories: [fixture.skillDirectory, conflictDirectory],
    });
    await assert.rejects(() => skills.refreshSkillDirectory(), /multiple configured directories/);
  } finally {
    await database.close();
    await removeSkillPackage(fixture.packageStore).catch(() => undefined);
    await fs.rm(fixture.root, { recursive: true, force: true });
...
test("repeated identical directories are canonicalized and deduplicated", async () => {
  const fixture = await createDirectoryFixture();
  const database = new AppDatabase(":memory:");
  try {
    const skills = new SkillService(database, {
      packageStoreRoot: fixture.packageStore,
      skillDirectory: fixture.skillDirectory,
      skillDirectories: [fixture.skillDirectory],
    });
    assert.deepEqual(skills.skillDirectories, [await fs.realpath(fixture.skillDirectory)]);
    const catalog = await skills.refreshSkillDirectory();
    assert.deepEqual(catalog.map((item) => item.name), ["directory-demo"]);
  } finally {
    await database.close();
