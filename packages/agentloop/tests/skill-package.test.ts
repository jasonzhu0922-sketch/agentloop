...
import { SKILL_AGENT_LOOP_METADATA_FIELDS } from "../src/skills/agentloop-metadata.ts";
...
test("AgentLoop metadata values are declared by the shared Skill contract", () => {
  assert.deepEqual(SKILL_AGENT_LOOP_METADATA_FIELDS.sourceKinds.values, [
    "api",
    "database",
    "dataset",
    "document",
  ...
  assert.equal(SKILL_AGENT_LOOP_METADATA_FIELDS.roles.required, true);
  assert.equal(SKILL_AGENT_LOOP_METADATA_FIELDS.artifactKinds.required, true);
  assert.equal(SKILL_AGENT_LOOP_METADATA_FIELDS.sourceKinds.type, "list");
});

test("AgentLoop sourceKinds reject business-specific values", async () => {
  const workspace = await fs.mkdtemp(join(tmpdir(), "agentloop-package-source-kind-metadata-"));
  try {
    const source = [
      "---",
      "name: city-carbon",
      "description: Evaluate uploaded project materials with a rubric.",
      "agentloop:",
      "  roles:",
      "    - source_provider",
      "  artifactKinds:",
    ...
    await assert.rejects(
      ...
      /agentloop\.sourceKinds contains unsupported source kind project-materials/,
    );
  } finally {
    await removeSkillPackage(workspace);
  }
...
test("installed package metadata can be refreshed from current package contents at startup", async () => {
  ...
  try {
    ...
    const refreshed = await skills.get(owner.user.id, installed.id);
...
test("the admitted step receives the unchanged package Skill and persists package verification events", async () => {
  ...
  try {
    ...
    const verified = (await runs.events(owner.user.id, run.id)).filter((event) => event.type === "skill.package.verified");
