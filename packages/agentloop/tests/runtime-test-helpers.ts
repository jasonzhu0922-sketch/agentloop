...
/**
 * Opaque owner identity for kernel tests. The kernel treats userId as an
 * opaque string and no longer enforces a users-table foreign key, so tests do
 * not need the application-level AuthService to mint one.
 */
export function testOwner(): { user: { id: string } } {
