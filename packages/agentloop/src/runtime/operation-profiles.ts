...
const OPERATION_PROFILES: readonly OperationProfile[] = [
  {
    ...
    planningRules: [
      ...
      "For conversation-only data analysis, final delivery normally needs no file receipt. But a count, grouping, ranking, distribution, or numeric aggregate that depends on a structured extraction must require coverage-backed derived aggregation evidence; explicit caveats alone never substitute for an available complete aggregate.",
    ...
    executionRules: [
      ...
      "For authorized uploaded CSV, XLSX, or XLSM sources, use extract_source_tables first when field-based analysis, grouping, counting, or schema fidelity is needed; do not reconstruct CSV from read_source text.",
      ...
      "For count, grouping, ranking, distribution, or numeric aggregate questions, derive the statistic from the structured artifact with the aggregate Tool and state its coverage. Do not conclude that the result cannot be determined while complete derived aggregation evidence is available.",
