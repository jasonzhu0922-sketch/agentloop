...
export class ComputerExecutor {
  ...
  async readFiles(
    files: ReadonlyArray<{ path: string; offset?: number; limit?: number; ranges?: readonly ReadLineRange[] }>,
  ...
  async writeFile(path: string, content: string, mode: WriteFileMode | boolean): Promise<{
    path: string;
    mode: WriteFileMode;
    writtenBytes: number;
    bytes: number;
    ...
    if (writeMode === "append") {
      ...
      if (targetStat === undefined) throw new AppError("NOT_FOUND", "File not found; use mode=\"create\" before appending", 404);
      if (!targetStat.isFile()) throw badRequest("Append target must be a regular file");
    }
    const flag = writeMode === "append" ? "a" : writeMode === "overwrite" ? "w" : "wx";
    try {
...
function structuredStdoutProjection(
  ...
  // The delivery candidate is the Skill-authored semantic result, not raw rows.
  // Preserve it even when unrelated structured payload fields force the command
  // stdout through the content-addressed projection path.
...
function directoryPriority(workspacePath: string): number {
  ...
  let best: number = BASENAME_LOOKUP_DIRECTORY_PRIORITY.length;
