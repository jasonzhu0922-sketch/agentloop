...
const SKILL_ROLE_VALUES = new Set<string>(SKILL_AGENT_LOOP_ROLE_VALUES);
const ARTIFACT_KIND_VALUES = new Set<string>(SKILL_AGENT_LOOP_ARTIFACT_KIND_VALUES);
...
function parseAgentLoopFrontmatter(lines: readonly string[], closing: number): SkillAgentLoopMetadata | undefined {
  ...
  const sourceKinds = (fields.get("sourceKinds") ?? [])
    .map((value) => parseAgentLoopSourceKind(value));
  const qaKinds = (fields.get("qaKinds") ?? [])
...
function parseAgentLoopSourceKind(value: string): SkillAgentLoopSourceKind {
  const normalized = normalizeAgentLoopString(value, "sourceKinds");
  if (SOURCE_KIND_VALUES.has(normalized)) return normalized as SkillAgentLoopSourceKind;
  throw invalidPackage(`agentloop.sourceKinds contains unsupported source kind ${normalized}`);
}

function parseAgentLoopQaKind(value: string): SkillAgentLoopQaKind {
  const normalized = normalizeAgentLoopString(value, "qaKinds");
  if (QA_KIND_VALUES.has(normalized)) return normalized as SkillAgentLoopQaKind;
  throw invalidPackage(`agentloop.qaKinds contains unsupported QA kind ${normalized}`);
}
