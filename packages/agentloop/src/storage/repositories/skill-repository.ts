...
export class SkillRepository {
  ...
  async listByOwner(ownerUserId: string): Promise<SkillRow[]> {
    return await this.connection.prepare(`SELECT ${SKILL_COLUMNS} FROM skills WHERE owner_user_id = ? ORDER BY name`)
      .all(ownerUserId) as unknown as SkillRow[];
  }

  async listPackageSkills(): Promise<SkillRow[]> {
    return await this.connection.prepare(`SELECT ${SKILL_COLUMNS} FROM skills WHERE source_kind = 'package' ORDER BY owner_user_id, name`)
      .all() as unknown as SkillRow[];
  }

  async listPackageSkillsWithoutSourceProvenance(): Promise<SkillRow[]> {
    return await this.connection.prepare(`
  ...
  async findByIdAndOwner(skillId: string, ownerUserId: string): Promise<SkillRow | undefined> {
    return await this.connection.prepare(`SELECT ${SKILL_COLUMNS} FROM skills WHERE id = ? AND owner_user_id = ?`)
  ...
  async updatePackageMetadata(input: {
    id: string;
    name: string;
    description: string;
    instructions: string;
