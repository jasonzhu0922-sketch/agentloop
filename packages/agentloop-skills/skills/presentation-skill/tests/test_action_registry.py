...
class ActionRegistryTests(unittest.TestCase):
    ...
    def test_materializes_best_judgment_intake_action(self) -> None:
        action = {
            ...
            "command": [
                ...
                str(self.workspace),
                "--packet",
                str(self.workspace / "deck_start_packet.json"),
                "--answers",
                str(self.workspace / "intake_answers.json"),
                "--answered-by",
                "best_judgment",
                "--use-recommended-defaults",
                "--report",
                str(self.workspace / "intake_apply_report.json"),
            ],
        }

        record = materialize_registered_action(action, repo=REPO, workspace=self.workspace)
        self.assertEqual(record["action_id"], "record_deck_intake_answers")
        self.assertEqual(record["parameters"]["answered_by"], "best_judgment")
        self.assertIs(record["parameters"]["use_recommended_defaults"], True)

        command = resolve_registered_action(record, repo=REPO, workspace=self.workspace)
        self.assertIn("--use-recommended-defaults", command)
        self.assertIn("--answered-by", command)
