...
For normal final delivery, build and audit the current artifact without adding
...
Use `--fail-on-planning-warnings` and `--fail-on-whitespace-warnings` only
when the user request, current Plan evidence contract, or a concrete delivery
rubric explicitly requires warning-free QA evidence.
...
After a final workspace build, create the final delivery audit:
...
figures/charts/tables. It expands to scaffold, auto-bind, render-free QA, and
overwrite flags. Run a rendered default build for final delivery once sources
and artifacts are stable; delivery readiness reports treat fast-first-pass
builds as `fast_first_pass_not_final` and recommend the default final build
...
16. Run `build_workspace.py --qa --visual-review --overwrite` before final delivery, and add strict warning flags only when explicitly required by the user, current Plan evidence contract, or delivery rubric.
