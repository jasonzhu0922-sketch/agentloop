...
from build_command_policy import default_build_command
...
def _recommended_delivery_action(
    *,
    ...
    final_delivery_build_command: list[str],
    ...
    if "visual_review_not_run" in warning_reasons or "visual_review_not_run" in blocking_reasons:
        return {
            ...
            "reason": (
                ...
                "did not run it. Run a final build with visual review before delivery."
            ),
    ...
    if final_build_reasons:
        return {
            ...
            "action_type": "run_command",
            "reason": (
                "The workspace is missing a final delivery build or the latest build is not "
                "a final delivery build. Run the final build before delivery."
            ),
            "command": final_delivery_build_command,
...
def main() -> int:
    ...
    strict_warning_gate_metadata = {
        ...
        "policy": (
            "Strict warning gates are optional delivery evidence. Missing strict "
            "warning flags are recorded here but do not change delivery_status "
            "unless the user, current Plan evidence contract, or delivery rubric "
            "explicitly requires warning-free QA evidence."
        ),
    }
    ...
    commands = {
        ...
        "final_delivery_build": default_build_command(workspace, qa=True),
        "strict_build": default_build_command(workspace, qa=True, strict_warnings=True),
        "visual_review_build": default_build_command(
            workspace,
            qa=True,
            visual_review=True,
    ...
    recommended_next_action = _recommended_delivery_action(
        ...
        final_delivery_build_command=commands["final_delivery_build"],
    ...
    report = {
        ...
        "blocking_reasons": blocking_reasons,
        "warning_reasons": warning_reasons,
        "strict_warning_gate_metadata": strict_warning_gate_metadata,
        "gates": gates,
