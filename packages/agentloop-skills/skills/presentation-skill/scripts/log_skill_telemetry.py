#!/usr/bin/env python3
"""Append per-build failure telemetry to a workspace-local JSONL log.
...
fixing at the skill level (a rule that fires across many decks is a guardrail
candidate). During AgentLoop execution the skill package root is read-only, so
the default log is stored under the active workspace build directory. Pass
`--log` explicitly when intentionally aggregating telemetry elsewhere.
...
def _default_log_path(workspace: str) -> Path:
    return (
...
def main() -> int:
    ...
    parser.add_argument(
        "--log",
        help="Log file path (default: <workspace>/build/skill_telemetry/failures.jsonl)",
    )
    ...
    if args.log:
        ...
        log_path = _default_log_path(args.workspace)
