...
def _assert_pre_intake_readiness(
    ...
    if next_action.get("action_type") != "run_command":
        failures.append(
            {
                "step": "pre_intake_readiness",
                "reason": "intake_next_action_not_runnable",
                "next_action": next_action,
            }
        )
    if next_action.get("action_id") != "record_deck_intake_answers":
        failures.append(
            {
                "step": "pre_intake_readiness",
                "reason": "intake_next_action_not_registered",
                "next_action": next_action,
            }
        )
    parameters = next_action.get("parameters") if isinstance(next_action.get("parameters"), dict) else {}
    if parameters.get("answered_by") != "best_judgment" or parameters.get("use_recommended_defaults") is not True:
        failures.append(
            {
                "step": "pre_intake_readiness",
                "reason": "intake_next_action_not_bounded_defaults",
                "parameters": parameters,
            }
        )
    if "scripts/apply_deck_intake_answers.py" not in str(next_action.get("display_command") or ""):
        failures.append(
            {
                "step": "pre_intake_readiness",
                "reason": "intake_next_action_missing_display_command",
                "next_action": next_action,
            }
        )
    inventory = (
