...
from build_command_policy import default_build_command, repeat_build_command
...
def _deck_intake_apply_command(
    *,
    workspace: Path,
    ...
    if intake_packet.get("exists"):
        command.extend(
            [
                "--packet",
                str(_workspace_path(workspace, str(intake_packet.get("path") or "deck_start_packet.json"))),
            ]
        )
    command.extend(
        [
    ...
    command.extend(["--report", str(workspace / "intake_apply_report.json")])
    return command
...
def _recommendations(
    *,
    ...
    strict_build_command = default_build_command(workspace, qa=True, strict_warnings=True)
    preflight_command = [
        "python3",
        ...
        recs.append(
            {
                ...
                "reason": "deck_start_packet.json exists but intake_answers.json has not been written; run the bounded best-judgment intake action once, then continue to source authoring. Do not search schemas or references for intake fields.",
                "intake_status": intake_status,
                "deck_start_packet": intake_packet.get("path", "deck_start_packet.json"),
                "intake_answers": intake_answers.get("path", "intake_answers.json"),
                "answer_template": deck_intake.get("answer_template", {}),
                "questions": deck_intake.get("questions", []),
                "command": _deck_intake_apply_command(
                    workspace=workspace,
...
def main() -> int:
    ...
    data_artifact_build_command = repeat_build_command(
        {
            "workspace": str(workspace),
            "scaffold_data_artifacts": True,
            "auto_bind_artifacts": True,
            "artifact_bind_mode": "lead",
            "qa": True,
            "skip_render": True,
            "overwrite": True,
        },
    ...
    fast_first_pass_build_command = default_build_command(workspace, qa=True, skip_render=True)
    ...
    next_commands = {
        ...
        "strict_build": default_build_command(workspace, qa=True, strict_warnings=True),
    ...
    intake_answers = deck_intake.get("answers") if isinstance(deck_intake.get("answers"), dict) else {}
    intake_status = str(deck_intake.get("status") or "").strip()
    if intake_answers.get("exists") or intake_status == "intake_answers_missing":
        next_commands["intake_answers_apply"] = _deck_intake_apply_command(
            workspace=workspace,
            intake_packet=intake_packet,
            intake_answers=intake_answers,
            use_recommended_defaults=intake_status == "intake_answers_missing",
        )
