...
from build_command_policy import repeat_build_command
...
def _build_workspace_command(args: argparse.Namespace, workspace: Path) -> list[str]:
    return repeat_build_command(args, workspace)
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Shortcut for the render-free data-to-evidence first pass: "
            "--scaffold-data-artifacts --auto-bind-artifacts --artifact-bind-mode lead --qa --skip-render "
            "--overwrite. Add fail-on-warning flags only when strict warning gates are explicitly required."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "source-planning issues. Use only when strict warning gates are explicitly required."
        ),
    )
    parser.add_argument(
        "--skip-preflight",
...
def main() -> int:
    ...
    if telemetry_script.exists() and args.qa:
        telemetry_log_path = workspace / "build" / "skill_telemetry" / "failures.jsonl"
        telem_cmd = [
            py,
