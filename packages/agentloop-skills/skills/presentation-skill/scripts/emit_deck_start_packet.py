...
from build_command_policy import default_build_command_text, shell_join  # noqa: E402
...
def _default_build_command(workspace_text: str, **kwargs: Any) -> str:
    return default_build_command_text(workspace_text, **kwargs)
...
def _visual_review_commands(workspace_text: str) -> list[str]:
    return [
        _default_build_command(workspace_text, qa=True, visual_review=True),
        _shell_join(
            [
                "python3",
    ...
    *,
    ...
    checklist: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "verify_commands": [
                _default_build_command(workspace_text, qa=True, skip_render=True)
            ],
        ...
        },
        {
            ...
            "phase": "delivery",
            "gate": "A final build and delivery audit prove current sources, QA counts, artifact acceptance, and PPTX fingerprint.",
            "proof": [
                f"{workspace_text}/build/build_workspace_report.json",
            ...
            "verify_commands": [
                _default_build_command(workspace_text, qa=True),
    ...
    *,
    ...
    return {
        ...
        "qa_gates": {
            ...
            "strict_warning_gates": (
                "only when the user, current Plan evidence contract, or "
    ...
    phases.extend(
        [
            ...
            },
            {
                ...
                "continue_when": [
                    "blocking overflow, overlap, and design errors are zero; warnings are recorded for optional polish",
            ...
            },
            {
                ...
                "owner": "main_agent",
                "action": "Run final build and delivery readiness; hand off when current-source QA, artifact acceptance, and PPTX fingerprint are proven.",
                "commands": [
                    _default_build_command(workspace_text, qa=True),
                    _shell_join(["python3", "scripts/report_delivery_readiness.py", "--workspace", workspace_text]),
                ...
                "continue_when": [
                    ...
                    "blocking source-planning and delivery errors are resolved; warnings are recorded for optional polish",
                ],
            },
        ]
    )
    ...
    return {
        ...
        "reproducibility_requirements": {
            ...
            "qa": [
                ...
                "delivery next-action handoff for the default final build, source edits, or explicitly required strict warning gates",
            ],
        },
        "acceptance_checklist": acceptance_checklist,
    }
