...
def _recommended_label(option: Any) -> str:
    if not isinstance(option, dict):
        return ""
    label = _answer_text(option.get("label"))
    return label.replace("(Recommended)", "").strip()


def recommended_default_answers(packet: Any, *, user_prompt: str = "") -> dict[str, Any]:
    """Build deterministic best-judgment intake answers from the packet card."""
    request = packet.get("request_user_input") if isinstance(packet, dict) else {}
    ...
    if isinstance(questions, list):
        for question in questions:
            if not isinstance(question, dict):
                continue
            question_id = _answer_text(question.get("id"))
            options = question.get("options")
            first_option = options[0] if isinstance(options, list) and options else {}
            answer = _recommended_label(first_option)
            if question_id and answer:
                answers.append({"id": question_id, "answer": answer})
    prompt = _answer_text(user_prompt)
    if prompt and not any(item.get("id") == "audience_context" for item in answers):
        answers.insert(
            0,
            {
                "id": "audience_context",
                "answer": f"Use the original request as the audience and objective context: {prompt}",
            },
        )
    return {
        "answers": answers,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="When the answers file is missing, write deterministic best-judgment answers from the deck-start packet.",
    )
    parser.add_argument(
    ...
    if args.use_recommended_defaults and not answers_path.exists():
        packet = _load_json(packet_path, {}) if packet_path is not None else {}
        defaults = recommended_default_answers(packet, user_prompt=args.user_prompt)
        _write_json_if_changed(answers_path, defaults, dry_run=args.dry_run)
